          ...
          <div className="flex flex-col gap-2.5 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex flex-col gap-2.5 lg:flex-row lg:items-center lg:gap-4">
              <div className="space-y-1">
                <h2 className="text-lg font-semibold tracking-tight text-stone-900 dark:text-stone-100">CPA 同步</h2>
                <p className="text-sm text-stone-500 dark:text-stone-400">查看本地账号与 CPA 远端状态，并执行双向同步。</p>
              </div>
              <div className="flex flex-wrap items-stretch gap-2">
                {([
                  { label: "账户总数", value: summary.total, color: "text-stone-900 dark:text-stone-100", ring: "border-stone-200 dark:border-stone-700" },
                  { label: "正常", value: summary.active, color: "text-emerald-600 dark:text-emerald-400", ring: "border-emerald-100 dark:border-emerald-900/40" },
                  { label: "限流", value: summary.limited, color: "text-amber-500 dark:text-amber-300", ring: "border-amber-100 dark:border-amber-900/40" },
                  { label: "异常", value: summary.abnormal, color: "text-red-400 dark:text-red-300", ring: "border-red-100/60 dark:border-red-900/40" },
                  { label: "禁用", value: summary.disabled, color: "text-stone-400 dark:text-stone-500", ring: "border-stone-200 dark:border-stone-700" },
                ] as const).map(({ label, value, color, ring }) => (
                  <div
                    key={label}
                    className={cn(
                      "min-w-[88px] rounded-2xl border bg-stone-50/80 px-3 py-2 shadow-[0_6px_18px_rgba(15,23,42,0.04)] dark:bg-stone-800/60",
                      ring,
                    )}
                  >
                    <div className="text-[11px] font-medium leading-none text-stone-400 dark:text-stone-500">{label}</div>
            ...
            <div className="grid grid-cols-1 gap-2 sm:flex sm:flex-wrap sm:items-center">
              <Button
                variant="outline"
                className="h-10 rounded-xl border-stone-200 bg-white px-4 text-stone-700 dark:border-stone-700 dark:bg-stone-800 dark:text-stone-300 dark:hover:bg-stone-700"
            ...
            <>
              <div className="grid gap-3 md:grid-cols-5">
                {([
                  ...
                  <div key={label} className="rounded-2xl border border-stone-100 bg-stone-50 px-4 py-4 dark:border-stone-700 dark:bg-stone-800/60">
                    <div className="text-xs font-medium text-stone-400 dark:text-stone-500">{label}</div>
              ...
              {syncView.lastRun ? (
                <div className="rounded-2xl border border-stone-100 bg-stone-50 px-4 py-4 dark:border-stone-700 dark:bg-stone-800/60">
                  <div className="mb-2 text-sm font-medium text-stone-700 dark:text-stone-300">最近一次同步结果</div>
                  <div className="grid gap-3 md:grid-cols-4">
                    {[
                      ...
                      <div key={label} className="rounded-2xl border border-stone-100 bg-white px-4 py-3 dark:border-stone-700 dark:bg-stone-900">
                        <div className="text-xs font-medium text-stone-400 dark:text-stone-500">{label}</div>
                        <div className="mt-2 text-lg font-semibold tracking-tight text-stone-900 dark:text-stone-100">{value}</div>
                      </div>
                    ))}
                  </div>
                  {syncView.lastRun.error ? (
                    <div className="mt-3 text-sm text-rose-600 dark:text-rose-300">{syncView.lastRun.error}</div>
                  ) : null}
                </div>
              ) : null}
              ...
              {syncView.accounts.length > 0 ? (
                <div className="rounded-2xl border border-stone-100 bg-stone-50 px-4 py-4 dark:border-stone-700 dark:bg-stone-800/60">
                  <div className="mb-3 text-sm font-medium text-stone-700 dark:text-stone-300">待处理文件</div>
                  <div className="flex flex-wrap gap-2">
                    {syncView.accounts

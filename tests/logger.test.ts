...
describe("server logger", () => {
  it("formats timestamps in Beijing time", () => {
    ...
    assert.equal(formatBeijingTimestamp(date), "2026-05-16 19:26:21");
  });
  ...
  it("rolls log file dates by Beijing day", () => {
    ...
    assert.equal(formatBeijingTimestamp(date), "2026-05-17 00:01:02");
  });
  ...
  it("uses concise Chinese step labels instead of English levels", () => {
    const line = formatLogLine(
      ...
      { status: 429, attempt: 1, retryAfterMs: 2000, nextDelayMs: 5000, bodyPreview: "rate limited" },
      new Date("2026-05-16T11:26:56.678Z"),
    );

    assert.equal(
      line,
      '[2026-05-16 19:26:56] [轮询异常] [openai-client] 轮询图片返回异常状态  {"status":429,"attempt":1,"retryAfterMs":2000,"nextDelayMs":5000,"bodyPreview":"rate limited"}\n',
    );

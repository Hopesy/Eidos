...
describe("account remote refresh service", () => {
  ...
  it("preserves local account state when refresh fails because of network timeout", async () => {
    const store = createMemoryDependencies(
      [createAccount({ access_token: "token-network", quota: 5, status: "正常" })],
      {
        async fetchRemoteAccountInfo() {
    ...
    const refreshed = await service.refreshAccountState("token-network");

    assert.equal(refreshed?.status, "正常");
    assert.equal(refreshed?.quota, 5);
    assert.equal(refreshed?.refresh_error, "网络异常，已保留本地账号状态，请检查网络后重试");
    assert.equal(refreshed?.refresh_error_reason, "network_error");
    assert.equal(store.record("token-network")?.status, "正常");
    assert.equal(store.record("token-network")?.quota, 5);
  });
  ...
  it("does not turn accounts abnormal when bulk refresh hits transient upstream errors", async () => {
    const store = createMemoryDependencies(
      [
        createAccount({ access_token: "token-a", quota: 4, status: "正常" }),
        createAccount({ access_token: "token-b", quota: 0, status: "限流" }),
      ],
      {
        async fetchRemoteAccountInfo(accessToken: string) {
          if (accessToken === "token-a") {
            throw new Error("/backend-api/me failed: HTTP 503");
          }
    ...
    const result = await service.refreshAccounts(["token-a", "token-b"], {
      markRefreshedAt: true,
    });
    ...
    assert.deepEqual(result.errors, [
      {
        ...
        error: "上游服务暂时不可用，已保留本地账号状态，请稍后重试",
        reason: "network_error",
      },
      {
        access_token: "token-b",
        error: "请求超时，已保留本地账号状态，请稍后重试",
        reason: "request_timeout",
      },
    ]);
    assert.equal(store.record("token-a")?.status, "正常");
    assert.equal(store.record("token-a")?.quota, 4);
    assert.equal(store.record("token-a")?.last_refreshed_at, null);
    assert.equal(store.record("token-b")?.status, "限流");
    assert.equal(store.record("token-b")?.quota, 0);
    assert.equal(store.record("token-b")?.last_refreshed_at, null);
  });
